...
export const run = async (processData: Function) => {
  if (!process.env.MONGO_DB_URI) {
    throw new Error(
      "Please provide a valid MongoDB URI to connect to the database"
    );
  }
  ...
  try {
    ...
    const insertedResponse = await collection.insertOne(ratesData);
    console.log("Record saved:", JSON.stringify(ratesData), insertedResponse);
  } finally {
    await client.close();
  }
};
